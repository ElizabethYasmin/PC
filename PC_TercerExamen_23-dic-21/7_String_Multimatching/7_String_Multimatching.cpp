...
struct aho_corasick {
    ...
    aho_corasick(vs keywords) {
        ...
        queue<go_node*> q; iter(a, go->next) q.push(a->second);
        while (!q.empty()) {
            ...
            iter(a, r->next) {
                ...
                while (st && st->next.find(a->first) == st->next.end())
                    st = st->fail;
                if (!st) st = go;s->fail = st->next[a->first];
                if (s->fail) {
                    if (!s->out) s->out = s->fail->out;
                    else {out_node* out = s->out;
                        while (out->next) out = out->next;out->next = s->fail->out;
                    }
                }
            }
        }
...
int main(){
    ...
    while (cin >> n){
        ...
        vs keywords;
        getline(cin,text);
        for (int i=0;i<n;i++){getline(cin,text); keywords.push_back(text);all[text] = i;}
        getline(cin,text);
        aho_corasick AC(keywords);
        vv res = AC.search(text);
        for (int i=0;i<n;i++){int ii = all[keywords[i]];
            for (int j=0;j<res[ii].size();j++)
                if (!j || (j && res[ii][j] != res[ii][j-1])) cout << res[ii][j] << " ";cout << endl;}
    }
   return 0;
}