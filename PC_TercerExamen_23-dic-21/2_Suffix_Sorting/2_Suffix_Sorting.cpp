...
struct SufArray {
    string s;int n;vv  P;
    SufArray(string &_s) : s(_s), n(_s.length()) { construct(); }
    void construct() {
        P.push_back(v(n, 0));
        compress();
        v  occ(n + 1, 0), s1(n, 0), s2(n, 0);
        for (int k = 1, cnt = 1; cnt / 2 < n; ++k, cnt *= 2) {
            P.push_back(v(n, 0));
            for (int i = 0; i <= n; ++i) occ[i] = 0;
            for (int i = 0; i < n; ++i)occ[i+cnt<n?P[k-1][i+cnt]+1:0]++;
            for (int i = 0, sum = 0, t; i <= n; ++i) {t = occ[i];occ[i] = sum;sum += t;}
            for (int i = 0; i < n; ++i)s1[occ[i+cnt<n?P[k-1][i+cnt]+1:0]++] = i;
            for (int i = 0; i <= n; ++i) occ[i] = 0;
            for (int i = 0; i < n; ++i)occ[P[k-1][s1[i]]]++;
            for (int i = 0, sum = 0, t; i <= n; ++i) {t = occ[i];occ[i] = sum;sum += t;}
            for (int i = 0; i < n; ++i)s2[occ[P[k-1][s1[i]]]++] = s1[i];
...
int main() {
    ...
    while (getline(cin, s)) {
        SufArray sa(s);
        v inv(s.length(), -1), &arr = sa.get_array();
        for (int i = 0; i < s.length(); ++i)inv[arr[i]] = i;
        int q;
        cin >> q;
        for (int c = 0; c < q; ++c) {int x;cin >> x;cout << inv[x] << ' ';}
        cout << endl;
        getline(cin, s);
    }return 0;
}